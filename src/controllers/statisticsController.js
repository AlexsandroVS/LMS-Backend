...
exports.getLowPerformanceStudentsByCourse = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      ...
      FROM Users u
      JOIN StudentEnrollments e ON u.UserID = e.StudentID
      JOIN CourseAssignments a ON e.AssignmentID = a.AssignmentID
    ...
    const [rows] = await pool.query(
      ...
      SELECT 
        ...
        (SELECT COUNT(DISTINCT e.StudentID) 
         ...
         JOIN CourseAssignments a2 ON e.AssignmentID = a2.AssignmentID
...
exports.getCoursesByTeacher = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      ...
      FROM Courses c
      JOIN 	CourseAssignments a ON c.CourseID = a.CourseID
      WHERE a.TeacherID = ?
      ORDER BY c.Title;
      `,
      [teacherId]
